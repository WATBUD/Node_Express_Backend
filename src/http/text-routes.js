...
export default service => {
  ...
  router.get('/resonances',async(req,res)=>{try{res.json({success:true,data:await service.resonances(req.user.user_id)})}catch(e){sendError(res,e)}})
  router.put('/profiles/:userId/resonance',async(req,res)=>{try{res.json({success:true,data:await service.resonate(req.user.user_id,req.params.userId,req.body.enabled)})}catch(e){sendError(res,e)}})
