...
try {
  ...
  assert.deepEqual(await call(`/api/chat/users/${a.id}/messages?afterId=${chat.id}`,b.token),[])
  const next=await call(`/api/chat/users/${b.id}/messages`,a.token,'POST',{text:'增量同步驗證'},201)
  const receiveStarted=performance.now()
  const delta=await call(`/api/chat/users/${a.id}/messages?afterId=${chat.id}`,b.token)
  assert.deepEqual(delta.map(m=>m.id),[next.id])
  console.log(`PASS chat incremental delivery; warmed HTTPS send ${sendMs}ms, receive ${Math.round(performance.now()-receiveStarted)}ms`)
  await call(`/api/chat/users/${a.id}/messages?afterId=bad`,b.token,'GET',undefined,400)
