...
class UserService {
  ...
  static async getUserById(ID) {
    try {
      const tableData = await SharedRepositoryInstance.getUserById(ID);
  ...
  static async updateUserPassword(userId, newPassword) {
    ...
    try {
      const existingUser = await SharedRepositoryInstance.prisma.users.findUnique({
        where: { user_id: parseInt(userId, 10) },
      });
      if (!existingUser) {
        throw new Error(`ID ${userId} 的用户不存在`);
      }
      updatedUser = await SharedRepositoryInstance.prisma.users.update({
        where: { user_id: parseInt(userId, 10) },
